...
//  Memory Defragmenter
//
//  Created by Pineapple 🍍 on 21.06.2025.
//
...
class ChromaDBAdapter {
    ...
    func saveOptimizedMemories(_ memories: [MemoryRecord], options: OptimizationOptions) async throws {
        ...
        # Get the old collection to find its UUID folder
        old_collections = client.list_collections()
        old_collection_ids = [str(col.id) for col in old_collections]
        print(f"Found {len(old_collection_ids)} existing collections: {old_collection_ids}", file=sys.stderr)
        
        # Delete existing collection
        try:
            client.delete_collection("memory_collection")
            print("Deleted memory_collection from ChromaDB", file=sys.stderr)
        except Exception as e:
            print(f"No existing collection to delete: {e}", file=sys.stderr)
        
        # Clean up ALL UUID folders before creating new collection
        print("Cleaning up ALL UUID folders before creating new collection...", file=sys.stderr)
        for item in os.listdir(chroma_path):
            item_path = os.path.join(chroma_path, item)
            # Check if it's a UUID folder (36 chars with dashes in right places)
            if (os.path.isdir(item_path) and 
                len(item) == 36 and 
                item[8] == '-' and item[13] == '-' and 
                item[18] == '-' and item[23] == '-'):
                try:
                    print(f"Removing UUID folder: {item}", file=sys.stderr)
                    shutil.rmtree(item_path)
                    print(f"Successfully removed: {item}", file=sys.stderr)
                except Exception as e:
        ...
        print(f"Created new collection with ID: {collection.id}", file=sys.stderr)
        ...
        if len(final_collections) != 1:
            print(json.dumps({
                'success': False,
                'error': f'Expected 1 collection after optimization, found {len(final_collections)}'
            }))
        else:
            # Verify only one UUID folder exists
            uuid_folders = [d for d in os.listdir(chroma_path) 
                          if os.path.isdir(os.path.join(chroma_path, d)) 
                          and len(d) == 36 and d[8] == '-' and d[13] == '-']
            
            print(f"Final UUID folders: {uuid_folders}", file=sys.stderr)
        ...
        if let count = json["count"] as? Int,
              let collectionId = json["collection_id"] as? String,
              let uuidFolders = json["uuid_folders"] as? Int {
            print("Successfully saved \(count) optimized memories to ChromaDB")
            print("New collection ID: \(collectionId)")
            print("UUID folders after optimization: \(uuidFolders)")
            
            if uuidFolders > 1 {
                print("WARNING: Multiple UUID folders detected. ChromaDB may not be fully cleaned up.")
            }
        } else {
